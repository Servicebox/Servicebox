  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        const productsWithImages = await Promise.all(data.response.items.map(async (product) => {
          const imageUrl = await fetchImage(authId, authKey, product.id); // Вызываем функцию fetchImage
          return {
            ...product,
            imageSrc: imageUrl,
          };
